...
## Input list
data_config_train = {'reso': [args.reso, args.reso], 'trimapK': [5, 5],
                     'noise': True}  # choice for data loading parameters
...
# Original Data
traindata = AdobeDataAffineHR(csv_file='Data_adobe/Adobe_train_data.csv', data_config=data_config_train,
...
for epoch in range(0, args.epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        fg, bg, alpha, image, seg, bg_tr, multi_fr = data['fg'], data['bg'], data['alpha'], data['image'], data['seg'], \
                                                     data['bg_tr'], data['multi_fr']

        fg, bg, alpha, image, seg, bg_tr, multi_fr = Variable(fg.cuda()), Variable(bg.cuda()), Variable(
        ...
        if i % step == (step - 1):
            print(
                ...
                epoch + 1, i + 1, netL / step, alL / step, fgL / step, fg_cL / step, al_fg_cL / step, elapse / step,
                elapse_run / step))
            netL, alL, fgL, fg_cL, al_fg_cL, elapse_run, elapse = 0, 0, 0, 0, 0, 0, 0
